...
export class RadioJackpotDrawService {
  ...
  async conductDraw(conductDrawDto: ConductDrawDto): Promise<DrawResult> {
    try {
      ...
      const eligibleTickets = await this.getEligibleTickets(session[0].stationId,  new Date(), new Date());
    ...
    let query = this.db
      ...
      .where(
        and(
          ...
          gte(radioTickets.createdAt, startDate),
          lt(radioTickets.createdAt, endDate),
  ...
  async getDrawStats(drawId: number): Promise<{
    ...
    const eligibleTickets = await this.getEligibleTickets(session.stationId, new Date(), new Date());
